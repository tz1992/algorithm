...
public static void main(String[] args) {
	...
	int arr[]=new int[stuNum];
	 ...
	 in.nextLine();
	 String[]stringss=new String[opNum];
	 for (int i = 0; i < stringss.length; i++) {
		stringss[i]=in.nextLine();
	}
	 in.close();
	 for (int i = 0; i < stringss.length; i++) {
	String[] temp=stringss[i].split(" ");
	if(temp[0].equals("Q")){
		 System.out.println(getMax(arr,Integer.parseInt(temp[1]),Integer.parseInt(temp[2])));
	 }else{
		 arr[Integer.parseInt(temp[1])-1]=Integer.parseInt(temp[2]);
	 }
